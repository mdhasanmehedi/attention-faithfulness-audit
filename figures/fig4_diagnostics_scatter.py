...
AUDIT_PATTERNS = [
    "attribution_faithfulness_seed{seed}.json",   # written by current audit script
    "audit_seed{seed}.json",                      # legacy name used in the repo
    "seed{seed}/attribution_faithfulness.json",
    "attribution_faithfulness_{seed}.json",
]

# Training output is spread across per-seed files and combined multi-seed files
# (e.g. train_seed7_999.json holds a `per_seed` array). Rather than guess at
# filenames, scan every train_*.json and index whatever seeds each one reports.
TRAIN_GLOBS = ["train_*.json", "metrics_*.json", "history_*.json"]

ACC_KEYS = ("test_accuracy", "test_acc", "accuracy", "final_test_accuracy")
F1_KEYS = ("test_macro_f1", "test_f1", "macro_f1", "macro_f1_score", "final_test_macro_f1")
...
def _norm(v):
    """Accept either a fraction or a percentage; return a fraction."""
    return v / 100.0 if v is not None and v > 1.0 else v


def index_training_metrics():
    """
    Scan every training-output JSON and build {seed: (acc, f1)}.

    Handles both layouts present in the repository:
      - one file per seed, with a top-level "seed" field
      - one file per run covering several seeds, with a "per_seed" array
    """
    import glob
    ...
    for pat in TRAIN_GLOBS:
        paths.extend(sorted(glob.glob(os.path.join(RESULTS_DIR, pat))))
    ...
    for path in paths:
        ...
        records = []
        if isinstance(j, dict) and isinstance(j.get("per_seed"), list):
            records = [r for r in j["per_seed"] if isinstance(r, dict)]
        elif isinstance(j, list):
            records = [r for r in j if isinstance(r, dict)]
        ...
        for rec in records:
            ...
            if seed is None:
                continue
            seed = int(seed)
            acc = _norm(_first(rec, ACC_KEYS, path))
            f1 = _norm(_first(rec, F1_KEYS, path))
            if acc is None or f1 is None:
                continue
            if seed in found and (abs(found[seed][0] - acc) > 1e-9
                                  or abs(found[seed][1] - f1) > 1e-9):
                sys.exit(
                    f"\nERROR: seed {seed} appears with different metrics in two files:\n"
                    f"  {sources[seed]}: acc={found[seed][0]:.6f} f1={found[seed][1]:.6f}\n"
                    f"  {path}: acc={acc:.6f} f1={f1:.6f}\n"
                    "Resolve which run is canonical before plotting.\n"
                )
            found[seed] = (acc, f1)
            sources[seed] = path
    ...
    if missing:
        sys.exit(
            f"\nERROR: no test accuracy / macro-F1 found for seed(s) {missing}.\n"
            f"Scanned {len(paths)} file(s) in '{RESULTS_DIR}' matching {TRAIN_GLOBS}.\n"
            f"Accepted accuracy keys: {ACC_KEYS}\n"
            f"Accepted macro-F1 keys: {F1_KEYS}\n"
            "Add the correct key names or glob at the top of this script.\n"
        )
    return found, sources


def load_all():
    metrics, sources = index_training_metrics()
    data = {}
