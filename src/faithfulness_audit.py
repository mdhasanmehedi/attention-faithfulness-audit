"""
faithfulness_audit.py -- PhishFormer

Faithfulness comparison of three explanation methods on the SAME sample,
using the SAME perturbation-masking protocol, so results are directly
comparable:
...
CHANGES FROM THE ORIGINAL SCRIPT (in response to pre-submission review):
  - The random-masking control now draws N_RANDOM_MASKS (default 20)
    independent random masks per URL and averages the resulting drop,
    instead of a single random mask per URL. A single-draw control has
    its own sampling variance; averaging over many draws gives a more
    stable estimate of "what a typical random 20% mask does" and is a
    fairer comparison against the (fixed, non-random) top-k mask.
  - run_faithfulness() and summarize() now also return/save the raw
    per-URL drop arrays (not just means/stds), so that bootstrap
    confidence intervals can be computed downstream without rerunning
    the audit. This includes the full (n_urls x n_random_masks) matrix
    of individual random-mask drops before averaging, so that the
    control's own sampling variance can be examined after the fact.
  - Output is written to results/attribution_faithfulness_seed<SEED>.json.
    The filename previously omitted the seed, so auditing a second
    checkpoint silently overwrote the first.

Reuses the existing trained checkpoint -- NO retraining required.

Usage:
  python3 src/faithfulness_audit.py
  python3 src/faithfulness_audit.py --n_samples 500 --seed 42
  python3 src/faithfulness_audit.py --n_samples 500 --seed 42 --n_random_masks 20
"""
...
MASK_RATIO       = 0.20   # fraction of characters masked
IG_STEPS         = 32     # integration steps for integrated gradients
N_RANDOM_MASKS   = 20     # NEW: independent random masks drawn per URL, averaged


# -- Attribution methods (UNCHANGED from original) -----------------------------
...
def run_faithfulness(method_name, attr_fn, model, sampled, test_ds,
                     device, rng, n_random_masks: int = N_RANDOM_MASKS):
    """
    Run the perturbation-masking faithfulness test for one attribution method.

    CHANGED: random_drops is now, per URL, the MEAN of n_random_masks
    independently drawn random masks (previously: a single random mask).
    This reduces the sampling variance of the random-masking control.

    Returns (top_drops, random_drops, bottom_drops), each shape (len(sampled),),
    plus per_url_random_draws: a (len(sampled), n_random_masks) array of the
    individual random-mask drops before averaging, retained for downstream
    bootstrap CI computation.
    """
    top_drops, random_drops, bottom_drops = [], [], []
    per_url_random_draws = []
    ...
    for count, idx in enumerate(sampled):
        ...
        # NEW: draw n_random_masks independent random masks for this URL,
        # instead of one, and record each individual draw's drop.
        draws = []
        for _ in range(n_random_masks):
            random_chars = rng.choice(seq_len, size=n_mask, replace=False)
            draws.append(orig_conf - masked_confidence(model, tokens, label, random_chars, device))
        per_url_random_draws.append(draws)
        random_drops.append(float(np.mean(draws)))
    ...
    return (np.array(top_drops), np.array(random_drops), np.array(bottom_drops),
            np.array(per_url_random_draws))


def bootstrap_ci(values: np.ndarray, n_boot: int = 10000, alpha: float = 0.05, seed: int = 0):
    """
    NEW: Percentile bootstrap CI for the mean of `values`.
...
def summarize(method_name, top, random_, bottom, per_url_random_draws=None):
    """Compute stats, Wilcoxon test, and bootstrap CIs; return result dict."""
    stat, p = wilcoxon(top, random_, alternative="greater")
    faithful = bool(p < 0.05)

    # NEW: bootstrap CI on the mean excess drop (top - random), the quantity
    # reported as the primary effect size throughout the paper.
    excess_per_url = top - random_
    excess_mean, excess_ci_lo, excess_ci_hi = bootstrap_ci(excess_per_url)

    logger.info(f"\n-- {method_name} --")
    logger.info(f"  Top-attr masking    : {top.mean():.4f} +/- {top.std():.4f}")
    logger.info(f"  Random masking      : {random_.mean():.4f} +/- {random_.std():.4f}  "
                f"(mean of {N_RANDOM_MASKS} draws/URL)")
    logger.info(f"  Bottom-attr masking : {bottom.mean():.4f} +/- {bottom.std():.4f}")
    logger.info(f"  Excess drop (top-random): {excess_mean:.4f}  "
                f"95% bootstrap CI [{excess_ci_lo:.4f}, {excess_ci_hi:.4f}]")
    logger.info(f"  Wilcoxon W          : {stat:.1f}")
    logger.info(f"  p-value (top>random): {p:.6f}")
    logger.info(f"  FAITHFUL? {'YES' if faithful else 'NO'} (alpha=0.05)")
    ...
    result = {
        ...
        "excess_drop_mean":    excess_mean,
        "excess_drop_ci_low":  excess_ci_lo,
        "excess_drop_ci_high": excess_ci_hi,
        "wilcoxon_statistic":  float(stat),
        "p_value":             float(p),
        "faithful":            faithful,
        "n_random_masks_per_url": N_RANDOM_MASKS,
        # NEW: raw per-URL arrays, retained for any further downstream analysis
        # (e.g. re-deriving a different CI method later without rerunning the audit).
        "raw_top_drops":       [float(x) for x in top],
        "raw_random_drops":    [float(x) for x in random_],
        "raw_bottom_drops":    [float(x) for x in bottom],
    }

    # The (n_urls x n_random_masks) matrix of individual random-mask drops,
    # before per-URL averaging. Collected by run_faithfulness() but previously
    # discarded here, which left the module docstring overstating what is saved.
    if per_url_random_draws is not None:
        result["raw_per_url_random_draws"] = [
            [float(v) for v in row] for row in per_url_random_draws
        ]
...
def main(seed: int = 42, n_samples: int = 500, csv_path: str = "data/raw/malicious_phish.csv",
          n_random_masks: int = N_RANDOM_MASKS):
    set_seed(seed)
    device = get_device()
    device = torch.device("cpu")
    logger.info(f"Device: {device_info(device)} (CPU forced for gradient stability)")
    logger.info(f"Random-masking control: {n_random_masks} independent draws per URL, averaged")
    ...
    logger.info(f"Sampled {len(sampled)} URLs\n")

    results = {}

    logger.info("="*55)
    logger.info("METHOD 1/3: Raw Attention")
    logger.info("="*55)
    t, r, b, raw_rand = run_faithfulness("Attention", attribution_attention,
                               model, sampled, test_ds, device,
                               np.random.default_rng(seed), n_random_masks)
    results["attention"] = summarize("Raw Attention", t, r, b, raw_rand)

    logger.info("\n" + "="*55)
    logger.info("METHOD 2/3: Input x Gradient")
    logger.info("="*55)
    t, r, b, raw_rand = run_faithfulness("InputxGrad", attribution_input_x_gradient,
                               model, sampled, test_ds, device,
                               np.random.default_rng(seed), n_random_masks)
    results["input_x_gradient"] = summarize("Input x Gradient", t, r, b, raw_rand)

    logger.info("\n" + "="*55)
    logger.info("METHOD 3/3: Integrated Gradients")
    logger.info("="*55)
    t, r, b, raw_rand = run_faithfulness("IntGrad",
                               lambda m, tok, tgt, dev: attribution_integrated_gradients(m, tok, tgt, dev, IG_STEPS),
                               model, sampled, test_ds, device,
                               np.random.default_rng(seed), n_random_masks)
    results["integrated_gradients"] = summarize("Integrated Gradients", t, r, b, raw_rand)

    logger.info("\n" + "="*80)
    logger.info("FAITHFULNESS COMPARISON SUMMARY")
    logger.info("="*80)
    logger.info(f"{'Method':<25} {'Top drop':>10} {'Random drop':>12} {'Excess':>9} {'95% CI':>18} {'p-value':>10} {'Faithful':>10}")
    logger.info("-"*80)
    for key in ["attention", "input_x_gradient", "integrated_gradients"]:
        r = results[key]
        ci_str = f"[{r['excess_drop_ci_low']:.3f},{r['excess_drop_ci_high']:.3f}]"
        logger.info(
            f"{r['method']:<25} {r['top_drop_mean']:>10.4f} "
            f"{r['random_drop_mean']:>12.4f} {r['excess_drop_mean']:>9.4f} {ci_str:>18} "
            f"{r['p_value']:>10.4f} {'YES' if r['faithful'] else 'NO':>10}"
        )
    logger.info("="*80)
    ...
    # Seed is part of the filename: the previous fixed name meant auditing a
    # second checkpoint silently overwrote the first.
    path = os.path.join(RESULTS_DIR, f"attribution_faithfulness_seed{seed}.json")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--n_random_masks", type=int, default=N_RANDOM_MASKS,
                         help="Number of independent random masks drawn per URL for the control (default 20).")
