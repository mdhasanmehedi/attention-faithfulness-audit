...
dataset (651,191 URLs). Guarantees that exact-duplicate URLs (10,072 rows
across 2,207 groups) stay entirely within a single split, never split
...
Run from the repository root, with the dataset placed at
data/raw/malicious_phish.csv (see README for the Kaggle download link).
Adjust CSV_PATH, URL_COL, LABEL_COL below if your file layout differs.
"""
...
URL_COL = "url"
LABEL_COL = "type"
TEST_FRAC = 0.15                   # matches the original 15% test split
VAL_FRAC = 0.15                    # matches the original 15% val split
