...
`make_arch_figure.py` (Figure 1) and `fig2_protocol_schematic.py` (Figure 2) are diagrams with no data inputs; they draw the architecture and the audit protocol respectively. Every hyperparameter in the Figure 1 diagram is transcribed from `src/models.py` and matches §4.1, and the parameter total it states, 1,791,108, is reproduced exactly by summing those layer shapes. `make_arch_figure.py` writes both a PNG and a vector PDF. `fig5` plots the per-character arrays for the single qualitative example, transcribed from `results/example_urls_full.json`.
...
| Figure 1 | `figures/make_arch_figure.py` (diagram; hyperparameters transcribed from `src/models.py`) |
